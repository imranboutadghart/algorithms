...
char	*malloc_padding(char *str, size_t *len_addr, int64_t *old_len_addr)
{
	int64_t	len;
	int		i;
	char	*new_str;
...
char	*insert_padding(char *str, size_t *len_addr)
{
	...
	i = 0;
	while((len + 1 + i) % 512 != 448)
		i++;
	for (int k = 8; k < i; k++)
		new_str[len + (k / 8)] &= ~(0b10000000 >> k % 8);
	for (int k = 0; k < 8; k++)
		new_str[len + (i / 8) + k] = *(((char *) &len) + k);
	return (new_str);
}
